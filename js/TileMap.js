...
var TileMapLoaderClass = Class.extend({
  ...
  draw : function() {//
    ...
    if (this.preCacheCanvasArray != null) {
      ...
      for (var q = 0; q < this.preCacheCanvasArray.length; q++) {
        ...
        var visible = this.intersectRect({
          top : r1.y,
          left : r1.x,
          bottom : r1.y + r1.h,
          right : r2.x + r2.w
    ...
    for (var layerIdx = 0; layerIdx < this.currMapData.layers.length; layerIdx++) {
      ...
      for (var tileIDX = 0; tileIDX < dat.length; tileIDX++) {
        ...
        if ((worldX + this.tileSize.x) < this.viewRect.x || (worldY + this.tileSize.y) < this.viewRect.y || worldX > this.viewRect.x + this.viewRect.w || worldY > this.viewRect.y + this.viewRect.h)
          continue;
      ...
      for (var layerIdx = 0; layerIdx < this.currMapData.layers.length; layerIdx++) {
        ...
        if (name == "collision") {
          ...
          for (var objIdx = 0; objIdx < lyr.objects.length; objIdx++) {
            ...
            if (lobj.type != "")
              continue;
            //don't handle poly-lines just yet...
            //test if this tile is within our world bounds
            var worldX = lobj.x;
            var worldY = lobj.y;
  ...
  preDrawCache : function() {
    ...
    for (var cc = 0; cc < this.preCacheCanvasArray.length; cc++) {
      ...
      var vRect = {
        top : this.preCacheCanvasArray[cc].y,
