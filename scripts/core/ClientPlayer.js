...
ClientPlayerClass = PlayerClass.extend({
  ...
  draw : function(fractionOfNextPhysicsUpdate) {
    ...
    if (this.name == gGameEngine.gPlayer0.name) {
      ...
      gRenderEngine.drawString(this.displayName, 'roboto', dPX - 31, dPY - 41, 18, {
        color : "black"
      });
      gRenderEngine.drawString(this.displayName, 'roboto', dPX - 30, dPY - 40, 18, {
        color : "green"
      });
    }
    ...
    gGameEngine.colorTintSprite_fcn(colors[this.team], this._drawLegMask, {
      player : this
    });
  ...
  _drawLegMask : function(ctx, settings) {
    var spt = settings.player._legSpriteMaskAnimList[settings.player._currLegAnimIndex].getCurrentFrameStats(); {
      ...
      var rotRadians = 0;
      //settings.player.legRotation * (Math.PI / 180.0);
      ctx.translate(dPX, dPY);
      ctx.rotate(rotRadians);

      settings.player._legSpriteMaskAnimList[settings.player._currLegAnimIndex].draw(0, 0, {
        ctx : ctx,
        noMapTrans : true
      });
  ...
  _drawPlayerAvatar : function(ctx, settings) {
    var spt = settings.player._legSpriteAnimList[settings.player._currLegAnimIndex].getCurrentFrameStats(); {
      ...
      settings.player._legSpriteAnimList[settings.player._currLegAnimIndex].draw(0, 0, {
        ctx : ctx,
        noMapTrans : true
      });
    ...
    //determine the proper rotation for the top
    {
      ...
      var rotRadians = -1.0 * Math.PI + settings.player.faceAngleRadians;
      // Initial sprite position faces up, rotate 1/4 turn clockwise first.
      ctx.translate(dPX, dPY);
      ctx.rotate(rotRadians);

      drawSprite("turret.png", 0, 0, {
        ctx : ctx,
        noMapTrans : true
      });
