...
PlayerClass = EntityClass.extend({
  ...
  init : function(inputx, inputy, settings) {
    ...
    var weapon0Class = Factory.getClass("ShotGun");
    //("ChainGun");//("ShotGun");//("MachineGun");//("BounceBallGun");
    this.weapons[0] = new weapon0Class();
    this.weapons[0].onInit(this);

    var weapon1Class = Factory.getClass("Shield");
    //("Landmine");
  ...
  update : function() {
    ...
    if (this.powerUpTime > 0)
      this.powerUpTime -= 0.05;
    //CLM attempt to make this 'seconds' accurate?
    else
      this.powerUpTime = 0;
    //}
  ...
  sendUpdates : function() {
    ...
    if (this.pInput)
      this.toOthers.q_input(this.pInput);
    if (IS_SERVER) {
      this.toAll.q_stats({
        health : this.health,
        energy : this.energy,
